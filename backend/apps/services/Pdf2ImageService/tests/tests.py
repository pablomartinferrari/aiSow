...
def test_pdf_processing(pdf_path):
    ...
    try:
        with open(pdf_path, "rb") as f:
            files = {"file": f}
            data = {"last_page": 25}  # Only process first 25 pages
            response = requests.post(
                "http://localhost:8000/api/v1/process-pdf", files=files, data=data
            )

        print(f"PDF processing (first 25 pages): {response.status_code}")
        if response.status_code == 200:
...
def main():
    ...
    if os.path.exists(pdf_path):
        ...
        print("2. Skipping PDF test (no test_data.pdf found)")
        pdf_ok = True
        print()
